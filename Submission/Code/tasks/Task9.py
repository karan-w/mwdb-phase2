...
class Task9:
        ...
        for x in Subjects:
            if x not in SeedNodeSet:
                ...
                P1_Teleportation_Discounting[x - 1] = (
                    PI_Value[x - 1] - P1_ReSeeding_Value) / (Transportation_Probability)
    ...
    def Generate_Output(self, Computed_Ranked_Matrix, Subjects, n, Personalized_PageRank, m, Seed_Set):
        ...
        for i in Subjects:
            ...
            Top_n_Subjects[Index_Subject] = (
                (np.array(Computed_Ranked_Matrix.loc[i]))[1:n]).tolist()
...
def main():
    ...
    Subject_Subject_Similarity_Matrix = pd.DataFrame(
        arr, columns=subjects, index=subjects)
    Subject_Index_Descending_Matrix = pd.DataFrame(
        data=Subject_Subject_Similarity_Matrix.columns.values[np.argsort(
            -Subject_Subject_Similarity_Matrix.values, axis=1)], columns=subjects, index=subjects
    )
    n_Similarity_Graph = task.Create_Similarity_Graph(
        subjects, Subject_Subject_Similarity_Matrix, Subject_Index_Descending_Matrix, n)

    Complete_Similarity_Graph = task.Create_Similarity_Graph(
        subjects, Subject_Subject_Similarity_Matrix, Subject_Index_Descending_Matrix)

    Personalized_PageRank = task.Compute_Personalized_PageRank(
        subjects, Subject_Subject_Similarity_Matrix, input_subjects)
    output = task.Generate_Output(
        Subject_Index_Descending_Matrix, subjects, n, Personalized_PageRank, m, input_subjects)

    OUTPUT_FILE_NAME = 'output.json'
    timestamp_folder_path = Output().create_timestamp_folder(
        args.output_folder_path)  # /Outputs/Task1 -> /Outputs/Task1/2021-10-21-23-25-23
