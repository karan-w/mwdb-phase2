...
class Task8:
    ...
    def setup_args_parser(self):
        ...
        parser.add_argument(
            '--subject_subject_similarity_matrix_filepath', type=str, required=True)
    ...
    def find_subjects_significance(self, S):
        S = S[~np.eye(S.shape[0], dtype=bool)].reshape(
            S.shape[0], -1)  # Remove the diagonals 6 * 5
    ...
    def find_m_significant_subjects(self, subjects_significance, m):
        subjects_significance_ascending_ranking = np.argsort(
            subjects_significance)  # Ranking from lowest to highest
        subjects_significance_descending_ranking = np.flip(
            subjects_significance_ascending_ranking)
    ...
    def save_output(self, args, subjects_significance, m_significant_subjects, similarity_graph):
        ...
        for subject_id, subject_significance in enumerate(subjects_significance):
            ranked_subjects_significance.append(
                (subject_id, subject_significance))

        ranked_subjects_significance.sort(key=lambda x: x[1])
        ranked_subjects_significance.reverse()
        similarity_graph_adjacency_matrix = graphmatrix.adjacency_matrix(
            similarity_graph.graph).toarray().tolist()
        output = {
            'args': {
        ...
        timestamp_folder_path = Output().create_timestamp_folder(
            args.output_folder_path)  # /Outputs/Task8-> /Outputs/Task8/2021-10-21-23-25-23

        similarity_graph.save(timestamp_folder_path)

        # /Outputs/Task8/2021-10-21-23-25-23 -> /Outputs/Task8/2021-10-21-23-25-23/output.json
...
def main():
    ...
    subject_subject_similarity_matrix, number_of_subjects = task.read_subject_subject_similarity_matrix(
        args.subject_subject_similarity_matrix_filepath)

    similarity_graph = SimilarityGraph()
    similarity_graph.create(
        number_of_subjects, subject_subject_similarity_matrix, args.n)
    ...
    m_significant_subjects = task.find_m_significant_subjects(
        subjects_significance, args.m)

    task.save_output(args, subjects_significance,
                     m_significant_subjects, similarity_graph)
